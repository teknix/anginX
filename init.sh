...
echo "  ─────────────────────────────────────────────────"
echo ""
echo "  curl — Docker (same compose network):"
echo -e "    ${CYAN}curl -X POST http://localhost/new \\${NC}"
echo -e "    ${CYAN}  -H 'Authorization: Bearer ${ANGINX_API_KEY}' \\${NC}"
echo -e "    ${CYAN}  -H 'Content-Type: application/json' \\${NC}"
echo -e "    ${CYAN}  -d '{\"domain\":\"app.yourdomain.com\",\"port\":7705,\"name\":\"myapp\"}'${NC}"
echo ""
echo "  curl — LAN (service on a different host):"
echo -e "    ${CYAN}curl -X POST http://YOUR_SERVER_IP/new \\${NC}"
echo -e "    ${CYAN}  -H 'Authorization: Bearer ${ANGINX_API_KEY}' \\${NC}"
echo -e "    ${CYAN}  -H 'Content-Type: application/json' \\${NC}"
echo -e "    ${CYAN}  -d '{\"domain\":\"app.yourdomain.com\",\"port\":7705,\"name\":\"myapp\",\"host\":\"192.168.1.50\"}'${NC}"
echo ""
echo "  Python — Docker (same compose network):"
echo -e "    ${CYAN}import urllib.request, json${NC}"
echo -e "    ${CYAN}urllib.request.urlopen(urllib.request.Request(${NC}"
echo -e "    ${CYAN}    'http://anginx/new',${NC}"
echo -e "    ${CYAN}    data=json.dumps({'domain':'app.yourdomain.com','port':7705,'name':'myapp'}).encode(),${NC}"
echo -e "    ${CYAN}    headers={'Content-Type':'application/json','Authorization':'Bearer ${ANGINX_API_KEY}'},${NC}"
echo -e "    ${CYAN}))${NC}"
echo ""
echo "  Python — LAN (service on a different host):"
echo -e "    ${CYAN}import urllib.request, json${NC}"
echo -e "    ${CYAN}urllib.request.urlopen(urllib.request.Request(${NC}"
echo -e "    ${CYAN}    'http://YOUR_SERVER_IP/new',${NC}"
echo -e "    ${CYAN}    data=json.dumps({'domain':'app.yourdomain.com','port':7705,${NC}"
echo -e "    ${CYAN}                    'name':'myapp','host':'192.168.1.50'}).encode(),${NC}"
echo -e "    ${CYAN}    headers={'Content-Type':'application/json','Authorization':'Bearer ${ANGINX_API_KEY}'},${NC}"
echo -e "    ${CYAN}))${NC}"
echo ""
echo "  ─────────────────────────────────────────────────"
